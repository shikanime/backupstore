...
func DecompressAndVerifyWithFallback(bsDriver BackupStoreDriver, blkFile, decompression, checksum string) (io.Reader, error) {
	...
	for {
		...
		if err == nil {
			r, derr := util.DecompressAndVerify(decompression, rc, checksum)
			_ = rc.Close()
			if derr == nil {
				return r, nil
			}
			...
			if strings.Contains(derr.Error(), gzip.ErrHeader.Error()) {
				alternativeDecompression = "lz4"
			} else if strings.Contains(derr.Error(), "lz4: bad magic number") {
				alternativeDecompression = "gzip"
			}

			if alternativeDecompression != "" {
				rc2, rerr := bsDriver.Read(blkFile)
				if rerr == nil {
					r2, derr2 := util.DecompressAndVerify(alternativeDecompression, rc2, checksum)
					_ = rc2.Close()
					if derr2 == nil {
						return r2, nil
					}
					err = errors.Wrapf(derr2, "fallback decompression also failed for block %v", blkFile)
				} else {
					err = rerr
				}
			} else {
				err = errors.Wrapf(derr, "decompression verification failed for block %v", blkFile)
			}
		}
